...
#     from src.logs.log_config import log_error as actual_log_error

#     return actual_log_error(*args, **kwargs)

def load_environmental_values() -> None :
    config_path = BASE_DIR / "src" / "config"
    config_files = [
        config_path / "find_config.env",
        config_path / "log_config.env",
        config_path / "master_config.env",
        config_path / "move_config.env",
        config_path / "remove_config.env",
    ]
    
    for config_file in config_files:
        if not os.path.exists(config_file):
            raise FileNotFoundError
        de.load_dotenv(dotenv_path= config_file)
...
def assign_environmental_to_variables() -> SimpleNamespace:
    ...
    return SimpleNamespace(
        ...
        LOG_PATH = _get_environmental_values("LOG_LOCATION"),
    )
